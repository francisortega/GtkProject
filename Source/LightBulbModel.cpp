...
LightBulbModel::LightBulbModel(Model* parent) : Model(parent)
{
	// Load the texture 
	Texture = Texture::loadDDS("../Source/Models/LightBulb/light.dds");
	...
	bool res = Renderer::LoadOBJ("../Source/Models/LightBulb/bulb.obj", vertices, uvs, normals);
...
void LightBulbModel::Update(float dt)
{

}

void LightBulbModel::Draw()
{
...
bool LightBulbModel::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
