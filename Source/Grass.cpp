...
Grass::Grass(Model* parent) : Model(parent)
{
	// Load the texture 
	Texture = Texture::loadDDS("../Source/Models/Grass/grass_texture.dds");
	...
	bool res = Renderer::LoadOBJ("../Source/Models/Grass/grass.obj", vertices, uvs, normals);
...
bool Grass::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
