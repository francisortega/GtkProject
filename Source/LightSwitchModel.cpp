...
LightSwitchModel::LightSwitchModel(glm::vec3 &lightBulbColor, Model* parent) : Model(parent)
{
	...
	// Load the texture 
	Texture = Texture::loadDDS("../Source/Models/LightSwitch/lightswitch.dds");
	...
	bool res = Renderer::LoadOBJ("../Source/Models/LightSwitch/lightswitch.obj", vertices, uvs, normals);
...
LightSwitchModel::~LightSwitchModel()
{
...
void LightSwitchModel::Update(float dt)
{

}

void LightSwitchModel::Draw()
{
...
bool LightSwitchModel::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
...
void LightSwitchModel::Picked()
{
	if (lightBulbColor->x > 0.0)
	{
		lightBulbColor->x = 0.0f;
		lightBulbColor->y = 0.0f;
		lightBulbColor->z = 0.0f;
	}
	else
	{
		lightBulbColor->x = 1.0f;
		lightBulbColor->y = 1.0f;
		lightBulbColor->z = 1.0f;
	}
}

std::vector<glm::vec3> LightSwitchModel::GetWorldVertices()
{
	return vertices;
}
