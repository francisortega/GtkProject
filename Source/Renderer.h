...
public:
	...
	static bool   LoadOBJ(const char * path, std::vector<glm::vec3> & out_vertices, std::vector<glm::vec3> & out_uvs, std::vector<glm::vec3> & out_normals );

	static unsigned int GetShaderProgramID() { return sShaderProgramID[sCurrentShader]; }
	static ShaderType GetCurrentShader() {
		if (sCurrentShader == 0){ return SHADER_PHONG; }
		else { return SHADER_FOG; }
	}
