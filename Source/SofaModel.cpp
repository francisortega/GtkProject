// SofaModel.cpp
// Purpose: A model representing a sofa.
//
...
SofaModel::SofaModel(Model* parent) : Model(parent)
{
	// Load the texture 
	Texture = Texture::loadDDS("../Source/Models/Sofa/sofa.dds");
	...
	bool res = Renderer::LoadOBJ("../Source/Models/Sofa/sofa.obj", vertices, uvs, normals);
...
void SofaModel::Update(float dt)
{
...
bool SofaModel::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
