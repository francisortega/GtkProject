...
RubiksCubeModel::RubiksCubeModel(Model* parent) : Model(parent)
{
	// Load the texture 
	Texture = Texture::loadDDS("../Source/Models/RubiksCube/rubiks-cube.dds");
	...
	bool res = Renderer::LoadOBJ("../Source/Models/RubiksCube/rubiks-cube.obj", vertices, uvs, normals);
...
void RubiksCubeModel::Update(float dt)
{
...
bool RubiksCubeModel::ParseLine(const std::vector<ci_string> &token)
{
	if (token.empty())
	{
		return true;
